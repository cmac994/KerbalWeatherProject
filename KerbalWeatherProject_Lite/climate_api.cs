...
namespace KerbalWeatherProject_Lite
{
    ...
    public class climate_api : MonoBehaviour
    {
        public read_climo _kwx_read;
        ...
        public void get_dims()
        {
            ...
            heights_3d = _kwx_read.get_height_data();
            ...
            latitude = _kwx_read.getLat();
            longitude = _kwx_read.getLng();
        }

        //Define a dictionary to store the index of the coordinate that most costly matches the position of the current vessel
        public Dictionary<string,int> get_dim_idx(double vlat, double vlng, double vheight)
        {
            double epoch_time = Util.getLocalTime();
        ...
        public double getAmbientTemperature(double vlat, double vlng, double vheight) { 
        ...
        public wx_aero getPTD(double vlat, double vlng, double vheight)
        {
            double epoch_time = Util.getLocalTime(); //Get local time (i.e. UT time)
            Dictionary<string, int> idx = get_dim_idx(vlat, vlng, vheight); //Get index of coordinates closest to vessel location
            int t = idx["t"]; int z = idx["z"]; int x = idx["x"]; int y = idx["y"];
        ...
        public wx_srf getAmbientWx2D(double vlat, double vlng)
        {
            double epoch_time = Util.getLocalTime(); //Get local time (i.e. UT time)
            Dictionary<string, int> idx = get_dim_idx(vlat, vlng, 0);  //Get index of coordinates closest to vessel location
        ...
        public double linear_interp(double[] arr_2d, int tidx, double epoch_time)
        {
        ...
        public double trilinear_interp_sfc(int vidx, int tidx, int lat_idx, int lng_idx, double epoch_time, double lat, double lng)
        {
        ...
        //Perform trilinear interpolation to get surface meteorological fields at a given time and position on Kerbin's surface (i.e. interpolate in longitude, latitude, and time)
        public double trilinear_interp_sfc3D(int vidx, int tidx, int lat_idx, int lng_idx, double epoch_time, double lat, double lng)
        {
            //Get temporal indices to interpolate between (loop back to zeroth index when temporal window extends into the next day)
            int tidx2;
            if (tidx == NT - 1)
        ...
        public double trilinear_interp(int vidx, int tidx, int hidx, int lat_idx, int lng_idx, double height, double lat, double lng)
        {
        ...
        public double interp_var3d(int vidx, int tidx, int hidx, int lat_idx, int lng_idx, double epoch_time, double height, double lat, double lng)
        {
            //Get temporal indices to interpolate between (loop back to zeroth index when temporal window extends into the next day)
            int tidx2;
            if (tidx == 5)
